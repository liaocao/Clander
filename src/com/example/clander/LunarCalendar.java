...
//这个数组就包含了从1900-2050年的农历信息
	...
	final private static int yearDays(int y) {
		int i, sum = 348;//348=360-12，这个就是默认所有月份都是29天，然后再确认30天的月份有多少个，就补起来了
		for (i = 0x8000; i > 0x8; i >>= 1) {//这个不是比较三次，因为这里是十六进制的，所以是从第16位一直比较到第5位
			if ((lunarInfo[y - 1900] & i) != 0)
				sum += 1;
		}
		return (sum + leapDays(y));//前面的sum就代表没有闰月的一年总天数
	}

	// ====== 传回农历 y年闰月的天数
	final private static int leapDays(int y) {
		if (leapMonth(y) != 0) {
			if ((lunarInfo[y - 1900] & 0x10000) != 0)//数组元素对应的17位就是表示闰大小月的
				return 30;
			else
				return 29;
		} else
	...
	}//取到y年对应的元素值，这个元素的1-4位就代表闰月的月份（如果没有润月就是0）

	// ====== 传回农历 y年m月的总天数
	final private static int monthDays(int y, int m) {
		if ((lunarInfo[y - 1900] & (0x10000 >> m)) == 0)//右移m，代表对应到m那个月
			return 29;
		else
			return 30;
	}
	...
	final public String animalsYear(int year) {
		final String[] Animals = new String[] { "鼠", "牛", "虎", "兔", "龙", "蛇",
				"马", "羊", "猴", "鸡", "狗", "猪" };
		return Animals[(year - 4) % 12];//减4是碰巧么，2015年是羊年，的确对应到了数组下标7
	}
	...
	/*
	*天干地支，简称为干支，源自远古时代对天象的观测。"甲、乙、丙、丁、戊、己、庚、辛、壬、癸"称为十天干，
	*"子、丑、寅、卯、辰、巳、午、未、申、酉、戌、亥"称为十二地支。天干地支组成形成了古代纪年历法。
	...
	*/
	...
	public static String getChinaDayString(int day) {
		...
		//如果day是10的倍数，那么n=9，否则n=day的余数减1，对应数组下标值chineseNumber[n]就是十
		if (day > 30)
			return "";
		if (day == 10)
			return "初十";
		else
			return chineseTen[day / 10] + chineseNumber[n];//返回农历日期
	}
	...
	public String getLunarDate(int year_log, int month_log, int day_log, boolean isday) {
		...
		Date nowaday = null;
		try {
			baseDate = chineseDateFormat.parse("1900年1月31日");//将基准日期转换为这种格式
		...
		try {
			nowaday = chineseDateFormat.parse(nowadays);//得到当前日期
		} catch (ParseException e) {
			e.printStackTrace(); // To change body of catch statement use
			// Options | File Templates.
		}
		...
		//86400000L是1天的时间24小时 x 60分钟 x 60秒 x 1000毫秒 单位是L
		dayCyl = offset + 40;
		monCyl = 14;
